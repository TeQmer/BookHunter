                    ...
                    try:
                        ...
                        if r.status_code == 200:
                            ...
                            for product in products:
                                ...
                                # Исключаем замки, сумки и другие не-книжные товары
                                excluded_words = [
                                    # Канцелярия
                                    "закладка", "канцеляр", "тетрад", "ручка", 
                                    "карандаш", "маркер", "стикер", "пенал",
                                    "набор канцеляр", "альбом для",
                                    "развивашка", "головоломк",
                                    "раскраск", "пропись", "азбука",
                                    # Замки и фурнитура
                                    "замок", "замоК", "замк",  # замки для дверей, чемоданов
                                    "петл", "ручк", "доводчик", "ограничитель",
                                    # Сумки и аксессуары
                                    "сумк", "портфел", "рюкзак", "чехол", "чехл",
                                    "кошелёк", "ключниц", "визитниц",
                                    # Одежда
                                    "футболк", "майк", "худи", "свитер", "куртк",
                                    "брюк", "джинс", "платье", "юбк",
                                    # Игрушки
                                    "игрушк", "конструктор", "пазл", "мягк игрушк",
                                    "машинк", "кукл", "лего",
                                    # Продукты
                                    "печенье", "конфет", "шоколад", "напиток",
                                    "чай", "кофе", "крупа", "макарон",
                                    # Электроника
                                    "телефон", "ноутбук", "планшет", "наушник",
                                    "зарядк", "кабель", "адаптер", "блок питания",
                                    # Бытовая химия
                                    "моющее", "чистящ", "стиральн", "порошок",
                                    "мыло", "шампунь", "гель",
                                    # Другое
                                    "инструмен", "крепеж", "болт", "гайк",
                                    "светильник", "люстр", "бра",
                                ]
                                is_excluded = any(word in name for word in excluded_words)
                                
                                # Проверяем что это книга (по названию или по entity)
                                is_book = ("книг" in name or "книжк" in name or 
                                          "книга" in name or "книжечк" in name or
                                          "book" in name or
                                          entity == "книги")
                                
                                # Также проверяем по расширенным данным - категории
                                extended = product.get("extended", {})
                                if extended and not is_book:
                                    categories = extended.get("categories", [])
                                    if categories:
                                        # Проверяем есть ли категория "Книги"
